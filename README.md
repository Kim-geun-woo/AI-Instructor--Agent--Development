...
* **LLM 프롬프트 엔지니어링**: LLM이 스크립트의 핵심 내용을 바탕으로 **자가진단용 객관식 또는 단답형 퀴즈**를 생성하도록 프롬프트를 설계했습니다. 강의의 핵심 키워드를 놓치지 않고 문제에 반영하는 데 집중했습니다.
...
* **Prompt Engineering (Quiz & Script)**: 단순히 LLM을 호출하는 것을 넘어, `Make_quiz`에서는 긴 스크립트에서 **핵심만 요약해 정형화된 퀴즈를 생성**하도록, `Gen_script_ctx`에서는 **이전 문맥을 참조해 자연스러운 연결어**를 만들도록 유도하는 프롬프트 엔지니어링의 중요성을 배웠습니다.
* **LangGraph 상태 관리**: `Make_quiz` 노드는 모든 루프가 끝난 후, `Gen_script_ctx`는 루프 *내부*에서 실행되어야 했습니다. `State`에 데이터를 누적(`total_script`)하거나 덮어쓰는(`previous_script_summary`) 방식을 이해하고, `conditional_edges`를 활용해 노드의 실행 시점을 제어하는 경험을 쌓았습니다.
...
* **학습자 경험(LX) 고려**: 퀴즈 기능은 단순한 '기능 추가'가 아니라, 사용자의 '능동적 학습 경험'을 위한 장치임을 이해했습니다. 에이전트 개발 시 최종 사용자의 경험을 고려하는 것의 중요성을 배웠습니다.
